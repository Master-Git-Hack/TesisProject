...
def bt():
    ...
    for address, name, device_class in devices:
        ...
        if len(services) == 0:
            discover.append({address: {'name': name, 'services': services, 'device_class': device_class}})
        else: discover.append({address: {'name': name, 'device_class': device_class}})
    return discover

def network(host):
    nmap = NmapScanTechniques()
    devices = nmap.nmap_fin_scan(host)
    return [{device: devices[device]} for device in devices if device != 'stats' and device != 'runtime']
